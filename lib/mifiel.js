...
export async function mifielCreateDocument({
  ...
  if (!payer && realSigners[0]?.email) payer = realSigners[0]?.email;
  safeAppend(form, "payer", payer, debugFields);
  ...
  // --- CRITICAL PATCH: .getBuffer() so axios always recognizes as buffer
  let formBuffer = null;
  try {
  ...
  // These block anything but real multipart
  const axiosOpts = {
    headers: {
      ...headers
      // no need for Content-Length, node-form-data will handle via boundary
    },
