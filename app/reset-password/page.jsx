...
export default function ResetPasswordPage() {
  ...
  const [tokenValid, setTokenValid] = useState(null); // null = checking, false = invalid
  ...
  useEffect(() => {
    ...
    if (!t) {
      setCheckError("El enlace de recuperación es inválido o ha expirado.");
      setTokenValid(false);
      return;
    }
    setToken(t);
    ...
    fetch("/api/auth/reset-password/check", {
      method: "POST",
      ...
      .then(async (res) => {
        if (!res.ok) {
          ...
          setCheckError(data?.error || "Enlace inválido o expirado.");
          setTokenValid(false);
        } else {
          setTokenValid(true);
          setCheckError("");
      ...
      .catch(() => {
        setCheckError("No fue posible verificar el enlace.");
        setTokenValid(false);
      });
  }, [params]);
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-cyan-100 dark:from-slate-900 dark:to-cyan-950 px-2">
      <div className="w-full max-w-md mx-auto bg-white/95 dark:bg-slate-900/95 rounded-lg shadow-lg p-10 flex flex-col items-center">
        ...
        {/* Token validation */}
        {tokenValid === null && (
          <div className="flex items-center text-slate-500 text-base py-6">
            <svg className="animate-spin w-6 h-6 mr-2 text-cyan-600" fill="none" viewBox="0 0 24 24">
              <circle
        ...
        {tokenValid === false && (
          <div className="flex flex-col items-center text-center mt-4">
            ...
            <div className="text-base text-gray-600 dark:text-gray-300">{checkError}</div>
            <a
